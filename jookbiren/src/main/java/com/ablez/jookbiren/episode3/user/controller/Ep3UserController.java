package com.ablez.jookbiren.episode3.user.controller;

import com.ablez.jookbiren.episode3.user.dto.UserDto;
...
@RestController
@RequestMapping("/ep3/users")
@Validated
@Slf4j
public class Ep3UserController {
...
//    public ResponseEntity login(@RequestBody @Valid CodeDto codeInfo, HttpServletResponse response) {
    ...
    public ResponseEntity getUserInfo(@RequestHeader("Authorization") String accessToken) {
        log.info("에피소드3 최종 유저 정보 조회 API 시작");
        userService.findCurrentUser(accessToken);
        InfoDto result = userService.getInfo();
        log.info("에피소드3 최종 유저 정보 조회 API 끝");
        return new ResponseEntity(result, HttpStatus.OK);
    }
